...
const LessonTemplate = ({ user }) => {
  ...
  useEffect(() => {
    const getQuestions = async () => {
      ...
      setAnswers(
        res.questions.map((ques) => {
          return {
            choice1: ques.incorrect_answer1,
            choice2: ques.incorrect_answer2,
          };
        })
      );
      setQuizId(res.id);
  ...
  function correct() {
    ...
    if (question < questionArray.length - 1) {
      ...
      setPop(!pop);
      fetch(`http://localhost:3000/users/${user.id}/list/add/${quizId}`, {
        method: "POST",
      });
    }
  }
  ...
  const choices = [
    <div>
      <AnswerChoice
        answer={answers[question].choice1}
        cbfunction={incorrect}
        lessonComplete={lessonComplete}
        pop={pop}
      />
    </div>,
    <div>
      <AnswerChoice answer={answers[question].choice2} cbfunction={incorrect} />
    </div>,
    <div>
      <AnswerChoice answer={correctAnswer[question]} cbfunction={correct} />
    </div>,
  ];
